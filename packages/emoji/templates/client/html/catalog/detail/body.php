                ...
                <div class="col-sm-6">
                    ...
                    <div class="catalog-detail-basic" aria-label="<?= $enc->attr( $this->translate( 'client', 'Product information' ) ) ?>">
                        ...
                        <div class="productEmoji">
                            <?php if (count($this->mostPopularEmojis)) : ?>
                                <span><?= $enc->html( $this->translate( 'client', 'Most popular reactions' ), $enc::TRUST ) ?>:</span>
                            <?php endif; ?>
                            <?php foreach( $this->mostPopularEmojis as $emoji ) : ?>
                                <div class="productEmoji">
                                    <?= str_replace('U+', '&#x', $emoji) ?>
                                </div>
                            <?php endforeach ?>
                        </div>
                        <div class="emojis">
                            <?php if ($this->loggedIn) : ?>
                                <?php if ($this->hasEmoji) : ?>
                                    <div class="emoji-popular" itemprop="<?= $this->hasEmoji->code ?>">
                                        <span><?= $enc->html( $this->translate( 'client', 'Your reaction' ), $enc::TRUST ) ?>:</span>
                                        <?= str_replace('U+', '&#x', $this->hasEmoji->code) ?>
                                    </div>
                                <?php else: ?>
                                    <span class="customer_reaction"><?= $enc->html( $this->translate( 'client', 'Tell us how you feel this product' ), $enc::TRUST ) ?>:</span>
                                    <?php foreach( $this->emojisList as $emoji ) : ?>
                                        <div class="emoji"
                                             onclick="sendCustomerEmojiReaction( '<?= $emoji->id ?>', '<?= $this->detailProductItem->getId() ?>' )"
                                             itemprop="<?= $emoji->id ?>">
                                                <?= str_replace('U+', '&#x', $emoji->code) ?>
                                        </div>
                                    <?php endforeach ?>
                                <?php endif; ?>
                            <?php endif; ?>
