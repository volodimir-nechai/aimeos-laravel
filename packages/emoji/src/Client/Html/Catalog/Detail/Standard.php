    ...
    implements \Aimeos\Client\Html\Common\Client\Factory\Iface
{
    ...
    public function data( \Aimeos\Base\View\Iface $view, array &$tags = [], string &$expire = null ) : \Aimeos\Base\View\Iface
    {
        ...
        $this->customerId = \Illuminate\Support\Facades\Auth::id();

        $this->getMostPopularProductEmojisList( $productItem->getId() );
        ...
        $view->emojisList = $this->getEmojisList();
        $view->hasEmoji = $this->checkCustomerEmojisForProduct( $productItem->getId() );
        $view->loggedIn = $this->checkLoggedIn();
        $view->mostPopularEmojis = $this->mostPopularEmojis;
    ...
    /**
     * Returns All Emojis Items
     *
     * @return \App\Models\MshopEmoji Emojis collection
     */
    protected function getEmojisList() : Object
    {
        return \App\Models\MshopEmoji::where( 'status','1' )->get();
    }

    /**
     * Returns most popular emojis for product
     *
     * @param string $product_id Product ID
     */
    protected function getMostPopularProductEmojisList( string $product_id ) : void
    {
        $productEmojisRows = \App\Models\MshopEmojiUserProductRelation::where( 'product_id', '=', $product_id)
            ->groupBy('emoji_id')
            ->orderByRaw('COUNT(*) DESC')
            ->take(3)
            ->get();

        if ($productEmojisRows && $productEmojisRows->count()) {
            foreach ($productEmojisRows as $productEmojisRow) {
                $this->mostPopularEmojis[] = \App\Models\MshopEmoji::where( 'id', $productEmojisRow->emoji_id )->pluck( 'code' )->first();
            }
        }
    }
     ...
     * @param string $product_id Product ID
     */
    protected function checkCustomerEmojisForProduct( string $product_id )
    {
        $userEmojiRow = \App\Models\MshopEmojiUserProductRelation::where( 'user_id', $this->customerId )->where( 'product_id', $product_id )->first();

        if ($this->customerId != null && $userEmojiRow && $userEmojiRow->count()) {
            return \App\Models\MshopEmoji::where( 'id', $userEmojiRow->emoji_id )->first();
        }
